...
import main.ZX81CalcInputGenerator;
import zxcompiler.ZXToken;

public class CBASICTokenizer {
	public enum BASICTokenTyp {
		isKeyword, isString, isNumber, isVariable, isSpace, isNull
	}
	...
	public static class BASICToken {
		...
		public String toString() {
			switch (typ) {
			case isNumber:
				return String.format("nr: %s", literal);
			case isString:
				return String.format("\"%s\"", literal);
			case isVariable:
				return String.format("var: %s", literal);
			case isKeyword:
				return String.format("token: %s %c", token, token);
			}
			return "unknown";
		}
	...
	public byte[] tokenize(String basic, int start) throws Exception {
		byte[] result;
		try {
	...
	public boolean nextToken(BASICToken token) {
		...
		if (Character.isAlphabetic(c)) {
			...
			while (mPos < len) {
				...
				if (c == ' ') {
					String s = "";
					if (token.literal.toUpperCase().equals("GO")) {
						for (int i = 1; i <= 4; i++)
							if (i + mPos < len)
								s += mLine.charAt(i + mPos);
					}
					s = s.toUpperCase();
					if (!(s.startsWith("TO") || s.startsWith("SUB")))
						break;
				} else if (!(c == '_' || c == '$' || Character.isAlphabetic(c) || Character.isDigit(c)))
					break;
				token.literal += c;
				mPos++;
			}
		...
		case '<':
			...
			if (c1 == '=') {
				mPos++;
				token.literal += c1;
				token.token = ZXToken.ZXB_SMALLEREQUAL;
			} else if (c1 == '>') {
				mPos++;
				token.literal += '>';
		...
		case '>':
			...
			if (c1 == '=') {
				token.literal += c1;
				mPos++;
				token.token = ZXToken.ZXB_BIGGEREQUAL;
			} else
				token.token = c;
			break;
		case '"':
