...
public class CCanvasSpriteMatrix extends Composite {
	...
	ISpriteCanvasData mData = null;
	ICanvasSpriteMatrixData mMatrix=null;
	...
	protected void onPaint(PaintEvent e) {
		...
		for (index = 0; index < mMatrix.getElemCount();index++) {
	...
	private void drawSpriteSub(GC gc, int index, int x, int y, int w, int h, int canvasw, int canvash, boolean color) {
		...
		byte[] bytes = mMatrix.getElem(index);
		int pos = 0;
			int sy = y;
			for (iy = 0; iy < h; iy++) {
				int sx = x;
				for (int ix=0;ix<w/8;ix++) {
					b = bytes[pos];
					pos++;
					mask = 0x80;
	...
	private int getTileW() {
		if (mData != null) return mMatrix.getElemW();
		return mTileW;
	}

	private int getTileH() {
		if (mData != null) return mMatrix.getElemH();
		return mTileH;
	}
	
	
	public void setData(ICanvasSpriteMatrixData matrix, ISpriteCanvasData canvasData) {
		mMatrix = matrix;
		mData = canvasData;
	}
